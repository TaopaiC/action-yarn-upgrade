...
2. Each module is upgraded in sequence. All major versions present in
   ...
   - `yarn up <module>@^<major>` — refreshes resolution to the latest in-range
     version
...
3. If `yarn.lock` changed for a module, it is staged and the change baseline is
   reset so the next module's check starts fresh.
