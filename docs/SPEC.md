...
對每個 module，先讀取 `yarn.lock` 找出所有已安裝的 major versions（例如同時存在
`^8` 和 `^9`），再對每個 major 依序執行：
...
> [!note] major version 邊界僅在同一 major 範圍內升級（patch /
> minor），不允許 major bump。若 yarn.lock 中同時有 `^8` 與
> `^9`，兩個範圍分別獨立升級。
...
> [!note] CVE vs GHSA若使用 `yarn npm audit --json`（Yarn Berry
> NDJSON 格式），CVE 欄位會顯示 GHSA ID（從 advisory
> URL 擷取）而非傳統 CVE 編號。

## 已決定的細節

- [x] Audit 解析：支援 classic npm audit JSON（`advisories` 物件）與 Yarn Berry
      NDJSON 兩種格式
